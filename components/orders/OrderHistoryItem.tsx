...
const OrderHistoryItem = ({ order }: Props) => {
  ...
  const statusBg = isDelivered ? 'bg-green-100' : 'bg-red-100';
    ...
    <View className="w-full md:w-1/2 lg:w-1/3 p-2">
      <View className={`rounded-2xl p-4 border ${
        ...
        <View className="flex-row justify-between items-center mt-2">
          <View className={`flex-row items-center px-2 py-1 rounded-md ${statusBg}`}>
            <Ionicons name={statusIcon} size={14} color={isDelivered ? '#16A34A' : '#DC2626'} />
          ...
          {order.actions.canReorder && (
            <View className="bg-orange-100 rounded-xl">
              <Pressable 
                style={({ pressed }) => ({ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 8, opacity: pressed ? 0.7 : 1 })}
              >
