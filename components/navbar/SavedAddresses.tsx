  ...
  return (
    <View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const isSelected = selectedAddress?.id === item.id || (!selectedAddress && item.isDefault);
            ...
            >
              <View
                className={`w-8 h-8 rounded-full items-center justify-center ${
                  isSelected ? 'bg-gray-200 dark:bg-gray-700' : 'bg-gray-100 dark:bg-gray-800'
                }`}
              >
                <Ionicons
                  name={getIconName(item.type)}
                  size={16}
                  color={colorScheme === 'dark' ? (isSelected ? '#D1D5DB' : '#9CA3AF') : '#4B5563'}
                />
              </View>
